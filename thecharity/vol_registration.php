...
if (isset($_POST['add'])) 
{
    $sql="select * from volunteer_register where email='".$_POST['inputEmail']."'";
...
if(empty($err) && empty($mailerr) && empty($fileerr) && empty($bloodgrouperr) && empty($passerr) && empty($conpaaerr) && empty($Adderr) && empty($cityerr) && empty($areaerr) && empty($birthdateerr))
{
    ...
    $sql = "insert into volunteer_register(firstname,email,password,contactno,address,city,area_name,bloodgroup,gender,birthdate,pic)values('$firstname','$email','$password','$mobile','$address','$city','$area','$bloodgroup','$gender','$birthdate','$folder')";
        ...
        if ($result && move_uploaded_file($temp_name, $folder))
        {
            ...
            $body = "please open link to verified ur email-" . $base_url . "?volunteer_id=".$last_id;
            echo $body;
            $mail = new PHPMailer;
